...
var fs, path, fsExtra, pug, livescript, stylus, jsYaml, marked, crypto, colors, minify, base, aux, bundle, hashstore, cwd, libpugs, pugbuild;
fs = require('fs');
path = require('path');
...
pugbuild.prototype = import$(Object.create(base.prototype), {
  libpug: function(){
    var i$, ref$, len$, spec;
    if (this._libpug != null) {
      return this._libpug;
    }
    for (i$ = 0, len$ = (ref$ = libpugs).length; i$ < len$; ++i$) {
      spec = ref$[i$];
      try {
        require.resolve(spec.replace(/^@\//, ''), {
          paths: [this.base]
        });
        return this._libpug = spec;
      } catch (e$) {}
    }
    return this._libpug = libpugs[0];
  },
  checkLibpug: function(){
    var mine, theirs, e, ver;
    mine = path.join(__dirname, '..', 'lib.pug');
    try {
      theirs = require.resolve(this.libpug().replace(/^@\//, ''), {
        paths: [this.base]
      });
    } catch (e$) {
      e = e$;
      return this.log.warn(("cannot resolve " + this.libpug() + " from " + this.base + ": " + e.message).yellow);
    }
    if (!fs.existsSync(mine) || path.resolve(mine) === path.resolve(theirs)) {
      return;
    }
    try {
      if (fs.readFileSync(mine).toString() === fs.readFileSync(theirs).toString()) {
        return;
      }
    } catch (e$) {
      e = e$;
    ...
    this.log.warn("the injected lib.pug is not the one shipped with this srcbuild.".yellow);
    this.log.warn(("  running : " + mine + " ( " + ver(mine) + " )").yellow);
    this.log.warn(("  injected: " + theirs + " ( " + ver(theirs) + " )").yellow);
    return this.log.warn("  it is resolved from the frontend root, so that copy wins.".yellow);
  },
    ...
    ret = import$(import$({}, this.locals || {}), {
      ...
      filters: import$(this.filters || {}, {
        ...
        'lsc': function(text, opt){
          ...
          return minify.orOriginal('js', code, {
            compress: false
          }, this$.log, (opt || {}).filename);
        },
        ...
        'stylus': function(text, opt){
          ...
          return minify.orOriginal('css', code, {}, this$.log, 'inline stylus');
        },
