...
specmgr.prototype = Object.create(Object.prototype) <<< do
  ...
  add: (o={}, opt = {}) ->
    ...
    @_[k] = s = new spec o
    s.codesrc.for-each (n) ~> @link codesrc: n, spec: r
    s.specsrc.for-each (n) ~> @link specsrc: n, spec: r
    if !opt.init => @set-dirty s
    s
  set: (o = {}, opt = {}) -> @add o, ({force: true} <<< opt)
  has-code: (f) -> @codesrc.has f
  touch-code: (f) ->
    f = if Array.isArray(f) => f else [f]
    keys = new Set!
    f.map ~>
      if !@codesrc[f] => return
  ...
  delete: (o = {}) ->
    r = @_[@key o]
    r.codesrc.for-each (n) ~> @unlink codesrc: n, spec: r
    r.specsrc.for-each (n) ~> @unlink specsrc: n, spec: r
    @set-dirty o
...
build = (o={}) ->
  ...
  @reldir = if typeof(opt.relative-path) == \string => opt.relative-path
  else if opt.relative-path and @cfgfn => path.dirname(@cfgfn)
  else process.cwd!
  @init({srcdir: 'static', desdir: 'static'} <<< opt)
  @

build.prototype = Object.create(Object.prototype) <<< do
  get-path: (f) ->
    if typeof(f) == \string => return f
    if @mgr => return @mgr.get-url(f)
    version = f.version or \main
    path = if f.path => f.path
    ...
    return path.join(@desdir, "assets/lib/#{f.name}/#version/#path")
  ...
  reset: ->
    ...
    @specmgr = new specmgr cachedir: @cachedir
  ...
  load-caches: ->
    ...
    traverse = (dir) ->
      ...
      ret = []
      for file in files =>
        if fs.is-directory file => ret ++= traverse(file)
  ...
  add-spec: (opts = []) ->
    ...
    opts.map (o) ~>
      files = o.[]files.map (f) ~> @get-path f
      @specmgr.update({} <<< o{name,type} <<< {codesrc: files, specsrc: [o.src]})
  ...
  des-path: ({name, type}) ->
    desdir = path.join(@desdir, \assets, \bundle)
    return
      desdir: desdir
